...
public class SelectContactAdapter extends ArrayAdapter<User> implements SectionIndexer{
    ...
    public SelectContactAdapter(Context context, ListView listView, int resource, List<User> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(convertView == null){
            ...
            if(res == 0) {
                convertView = layoutInflater.inflate(R.layout.view_select_contact_item, parent, false);
            } else {
                convertView = layoutInflater.inflate(res, null);
            }
            holder.checked = convertView.findViewById(R.id.iv_check_box);
        ...
        String header = CommonUtils.getInitialLetter(user);

        if (position == 0 || header != null && !header.equals(CommonUtils.getInitialLetter(getItem(position - 1)))) {
        ...
        if (listView.isItemChecked(position)) {
            holder.checked.setImageResource(R.drawable.ic_checked_24dp);
        } else {
            holder.checked.setImageResource(R.drawable.ic_uncheck_black_24dp);
        }

        if (holder.nameView != null) {
            holder.nameView.setText(userName);
        }
        Glide.with(getContext()).load(R.drawable.ic_account_circle_48px).into(holder.avatar);
    ...
    public Object[] getSections() {
        ...
        for (int i = 1; i < count; i++) {
            ...
            String letter = CommonUtils.getInitialLetter(getItem(i));
